...
Date_Parser = function(Date) { 
  ...
  # find location of 1/1, everything before prepend 2020
  
  
  
  
  # works, but slow
  # TODO: evaluate performance difference between lookup table and individual assignment
  md_dates = which(!is.na(as.Date(clean_dates, format = '%m-%d')))
  clean_dates[md_dates] = format(as.Date(clean_dates[md_dates], '%m-%d'), '%Y-%m-%d')
  clean_dates = as.Date(format(as.Date(clean_dates), '%Y-%m-%d'))
  fix_years = sapply(clean_dates, function(x) x = ifelse(x > Sys.Date(), seq(x, length=2, by="-1 years")[2], x))
  
  dates_out = format(as.Date(fix_years), '%Y-%m-%d')
  
  
  
  # fix md dates erroneously assigned to 2021
  # dates_out = format(as.Date(test, '%Y-%m-%d'))
...
# # names(DSHS_cases_time) = DSHS_cases_time[2, ]
# DSHS_cases_time = DSHS_cases_time[3:(nrow(DSHS_cases_time) - 11), ]
# names(DSHS_cases_time)[1] = 'County'
# 
# # convert wide data to long
# DSHS_cases_long = reshape::melt(DSHS_cases_time, id = c('County'))
# names(DSHS_cases_long) = c('County', 'Date', 'Cases_Cumulative')
# 
...
# 
# # fix colnames
# names(DSHS_deaths_time) = DSHS_deaths_time[2, ]
# last_row = grep('Zavala|ZAVALA', DSHS_deaths_time$County)
# 
# DSHS_deaths_time = DSHS_deaths_time[3:last_row, ]
# names(DSHS_deaths_time)[1] = 'County'
# 
# names(DSHS_deaths_time) = trimws(gsub('Fatalities', '', names(DSHS_deaths_time)))
# 
# dates_raw = names(DSHS_deaths_time)[2:length(names(DSHS_deaths_time))]
# 
...
# 
# # replace date values
# names(DSHS_deaths_time)[2:ncol(DSHS_deaths_time)] = dates
# # melt
# DSHS_deaths_long = reshape2::melt(DSHS_deaths_time, id = c('County'))
# names(DSHS_deaths_long) = c('County', 'Date', 'Deaths_Cumulative')
# 
...
# if (length(grep('.x', names(old_active_cases)) > 0)) { 
#     old_active_cases = old_active_cases[, -grep('\\.y', names(old_active_cases))]
#     names(old_active_cases) = gsub('\\.y', '', names(old_active_cases))
# }
# 

# names(old_active_cases)[2:ncol(old_active_cases)] = Date_Parser(names(old_active_cases)[2:ncol(old_active_cases)])
...
# write.csv(old_active_cases_long,
#           'original-sources/historical/active-cases/active_case_archive.csv',
#           row.names = FALSE)
...
names(dshs_pops) = c('County', 'Population_DSHS')
...
Get_TPR = function() { 
  ...
  tpr_date = Date_Parser(as.Date(str_match(tpr_df[2,2], '-(.*)')[2], '%B %d'))
  print(tpr_date)
...
# write.csv(tpr_df, paste0('original-sources/historical/cms_archive/cms_tpr_archive.csv'),
#             row.names = FALSE) 
...
# clean newest file
file_url = 'https://beta.healthdata.gov/api/views/gqxm-d9w9/files/ed514221-8fb0-4ba9-84b2-27282646efad?download=true&filename=Community_Profile_Report_20210113_Public.xlsx'
temp = tempfile()
download.file(file_url, temp, mode = 'wb')

cpr_date = read_xlsx(temp, sheet = 6) %>%
  slice(1) %>% 
...
# cms_archive = read.csv('original-sources/historical/CMS/cms_tpr_archive.csv')
...
<!--     names(clean_df)[1:3] = c('reporting_hospitals', 'total_hospitals', 'percent_reporting') -->
<!--     names(clean_df)[1:3] = paste0('HHS_', names(clean_df)[1:3]) -->

<!--     # drop date from colnames (will likely change over time) -->
<!--     names(clean_df) = gsub('_*\\d', '', names(clean_df)) -->
...
<!--   names(clean_df)[c(3:4, 6:7)] = paste0(data_type, names(clean_df[c(3:4, 6:7)])) -->
<!--   names(clean_df) = paste0('HHS_', names(clean_df)) -->
...
<!-- ```{r} -->
<!-- tryCatch({ -->
<!--   DSHS_tests = DSHS_state[[3]] -->

<!--   # # LAB TESTS -->
<!--   # select lab tests & remove rows where all rows are NA -->
<!--   DSHS_molecular_tests = DSHS_tests[2:nrow(DSHS_tests), c(1, 3, 4, 5)] %>% -->
<!--     setNames(c('Date', 'Molecular_tests_Daily', 'LAB_Positive_Tests_Daily','LAB_Positivity_Rate')) %>% -->
...
<!--   # # LAB TESTS -->
<!--   # select lab tests & remove rows where all rows are NA -->
<!--   DSHS_molecular_tests = DSHS_tests[2:nrow(DSHS_tests), 1:4] %>% -->
<!--     setNames(c('Date', 'molecular_tests_Daily', 'LAB_Positive_Tests_Daily','LAB_Positivity_Rate')) %>% -->
<!--     filter(rowSums(is.na(.)) < ncol(.)) %>% -->
<!--     mutate(Date = as.Date(as.integer(Date), origin = '1899-12-30')) -->
<!--   # # SPECIMEN TESTS -->
...
<!-- list(DSHS_specimen_tests, DSHS_molecular_tests, DSHS_hospitalizations)) -->
<!-- ``` -->
