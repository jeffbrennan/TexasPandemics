...
nyt_schools = rbindlist(lapply(list.files('original-sources/historical/nyt-colleges/', full.names = TRUE), read.csv)) %>% 
...
# download files posted either with _ or - before date
tryCatch({ 
...
county_isd_wide = read.csv('original-sources/county_isd_wide.csv')

DSHS_schools_clean = DSHS_schools_raw[first_row:last_row, ] %>%
  mutate(LEA = as.numeric(gsub("'", '', LEA))) %>%
  merge(., county_isd_wide, by.x = 'LEA', by.y = 'LEA', all.x = TRUE) %>%
  # mutate(District.x = District.y) %>% 
  dplyr::select(-District.y) %>% 
  ...
  setNames(c('District', 'Date', 'County', 'LEA', 'Total_Enrollment', 'Approximate_Enrollment',
...
DSHS_school_df = DSHS_school_df %>% 
  ...
  distinct() %>%
  dplyr::select(-LEA, -County)


write_xlsx(list('school_data' = DSHS_school_df,
                'helper' = read.csv('original-sources/county_isd_long.csv')),
...
DSHS_state_day = data.frame(Antigen_Tests_Total = DSHS_state[[5]][1,2],
...
```{r}
write.csv(nyt_schools, file = 'original-sources/nyt_colleges.csv', row.names = F)
```
