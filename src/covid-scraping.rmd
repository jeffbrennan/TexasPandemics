...
state_demo = lapply(all_dashboard_files, `[[`, 'By Age, Gender, Race') %>%
  ...
  mutate(`Age Group` = ifelse(`Age Group` == '44327', '5-11 years', `Age Group`)) %>%
  mutate(`Age Group` = ifelse(`Age Group` == '44545', '12-15 years', `Age Group`)) %>%
...
DSHS_hosp_clean = function(df, var_name) { 
  ...
  names(df_clean)[2:length(names(df_clean))] = dates
...
if (date_out %in% demo_releases) {
  ...
  #                                      col_option = FALSE)
  daily_demo_stack = rbindlist(lapply(names(weekly_demo), function(x) Clean_Demographics(x, m_date)), fill = TRUE) %>% 
