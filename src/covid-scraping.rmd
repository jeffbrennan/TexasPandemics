...
## FULL
  ...
  age_archive = rbindlist(lapply(demo_archive %>% .[str_detect(., 'age')], read_csv)) %>% distinct()
  race_archive = rbindlist(lapply(demo_archive %>% .[str_detect(., 'race')], read_csv)) %>% distinct()
  
  vax_age_full = rbindlist(lapply(list.files(glue('{vax_dir}demo_age_full/'), full.names = TRUE),
                                  read.csv, fileEncoding="UTF-16LE", sep = '\t'),
                           fill = TRUE) %>%
    dplyr::select(-'X.1') %>%
  ...
  vax_race_full = rbindlist(lapply(list.files(glue('{vax_dir}/demo_race_full/'), full.names = TRUE),
                               Read_Race),
                         fill = TRUE) %>%
...
# PARTIAL
  vax_age_partial = rbindlist(lapply(list.files(glue('{vax_dir}demo_age_partial/'), full.names = TRUE),
                                  read.csv, fileEncoding="UTF-16LE", sep = '\t'),
                           fill = TRUE) %>%
    dplyr::select(-'X.1') %>%
    reshape2::melt(id = c('X', 'X.2')) %>%
    setNames(c('Age', 'County', 'Gender', 'All_Vaccinated')) %>%
    mutate(Date = max(vax_admin$Date)) %>%
    mutate(County = gsub(' County', '', County)) %>%
    dplyr::select(Date, County, everything()) %>% 
    mutate(Date = as.Date(Date)) %>%
    mutate(All_Vaccinated = as.numeric(gsub(',', '', All_Vaccinated))) %>%
    arrange(Date, County)


  vax_race_partial = rbindlist(lapply(list.files(glue('{vax_dir}/demo_race_partial/'), full.names = TRUE),
                               Read_Race),
                         fill = TRUE) %>%
  ...
  vax_out = vax_allocation %>% 
    ...
    full_join(vax_age_partial %>% 
                ...
                summarize(All_Vaccinated = sum(All_Vaccinated, na.rm = TRUE))) %>% 
    ...
    dplyr::select(Date, County, Doses_Allocated_1, Doses_Allocated_2,
                  Doses_Administered, All_Vaccinated, Fully_Vaccinated,
                  everything())
