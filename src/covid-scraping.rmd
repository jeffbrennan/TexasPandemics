...
write_csv(x = variant_df,
          file = glue('original-sources\\historical\\variants\\tsa_variants_{Sys.Date()}.csv'))


all_variants = list.files('original-sources\\historical\\variants\\', full.names = TRUE) %>%
  lapply(., function(x) fread(x) %>%
           setNames(gsub('\\r\\n        ', '', names(.))) %>% 
           setNames(gsub('DELTA\\(', 'DELTA \\(', names(.)))
         )  %>%
  rbindlist(fill = TRUE) %>% 
...
DSHS_tsa = merge(DSHS_tsa_counts, DSHS_tsa_pops, by = 'TSA', all = TRUE)
