...
plant_offsets = c('', seq(2000, 10000, by = 2000))
zip_offsets = c('', seq(1000, 10000, by = 1000))
...
# biobot
biobot_ww_date = seq(as.Date('2022-11-10'), as.Date('2025-01-01'), by = 'week') %>%
  format(., '%Y-%m-%d') %>%
  .[which(. <= Sys.Date())] %>%
  .[length(.)]


biobot_ww = fread(glue('https://d1t7q96h7r5kqm.cloudfront.net/{biobot_ww_date}_automated_csvs/wastewater_by_county.csv'))
bioboot_ww_texas = biobot_ww %>%
  filter(state_abbr == 'TX') %>%
  mutate(date = format(date, '%Y-%m-%d')) %>%
  select(date, display_name, eff_conc_sarscov2_weekly_rolling, eff_conc_sarscov2_weekly) %>%
  mutate(display_name = str_replace_all(display_name, " County, TX", "")) %>%
  rename(Date = date,
         County = display_name
         )
fwrite(bioboot_ww_texas, 'tableau/wastewater_county_biobot.csv')


biobot_ww_variant_dates = seq(as.Date('2022-10-27'), as.Date('2025-01-01'), by = 'week')
biobot_ww_variant_date = biobot_ww_variant_dates[c(1, 4:length(biobot_ww_variant_dates))] %>%
  format(., '%Y-%m-%d') %>%
  .[which(. <= Sys.Date())] %>%
  .[length(.)]

biobot_ww_variant = fread(glue('https://d1t7q96h7r5kqm.cloudfront.net/{biobot_ww_variant_date}_variant_data/wastewater_variant_county.csv'))

biobot_ww_variant_texas = biobot_ww_variant %>%
    mutate(tx_county = str_detect(display_name, ', TX')) %>%
    filter(tx_county) %>%
    select(date, display_name, variant, var_pct) %>%
...
fwrite(biobot_ww_variant_texas, 'tableau/wastewater_county_variant_biobot.csv')
...
# PREV_VITAL_PATH = 'https://raw.githubusercontent.com/jeffbrennan/TexasPandemics/master/tableau/county.csv'
# PREV_VITAL_PATH = 'https://raw.githubusercontent.com/jeffbrennan/TexasPandemics/df1de69d2e04aece67dde310649eddf6bd6944d6/tableau/county.csv'
# PREV_VITAL_PATH = 'https://raw.githubusercontent.com/jeffbrennan/TexasPandemics/bb113d92aaff5da93ca472ffbd2e664cea7cea9c/tableau/county.csv'
# PREV_VITAL_PATH = 'https://raw.githubusercontent.com/jeffbrennan/TexasPandemics/977afe651ab4370bd46dbc6f1d125050f4c0d074/tableau/county.csv'
PREV_VITAL_PATH = 'tableau/county.csv'


# VITALS_DATE = '2022-09-16'
# VITALS_DATE = '2022-09-07'
...
# PREV_VITALS |> filter(County == 'Bexar') |> View()
```
...
DSHS_vitals_long_impute_prep = DSHS_vitals_long_raw |>
  ...
  mutate(missing_day = ifelse(missing_day == lag(missing_day) | missing_day == 0 | row_number() == 1, 0, missing_day)) |>
  mutate(impute_group = missing_day != 0 | lead(missing_day) != 0 | lag(missing_day) != 0) |>
...
IMPUTED_CHECK = DSHS_vitals_long_impute_prep |> filter(impute_group) |> nrow() > 0
...
if (IMPUTED_CHECK) {
  ...
  # DSHS_vitals_long_imputed |> filter(County == 'Bexar') |> View()
  ...
  DSHS_vitals_long =
    ...
    group_by(County) |>
    # mutate(Cases_Cumulative_Imputed = ifelse(Date %in% MISSING_VITALS_DATES,
    #                                          Cases_Cumulative + ((lead(Cases_Cumulative) - lag(Cases_Cumulative)) / 2),
    #                                          Cases_Cumulative_Imputed
    #                                          )
    #       ) |>
    # mutate(Deaths_Cumulative_Imputed = ifelse(Date %in% MISSING_VITALS_DATES,
    #                                Deaths_Cumulative + ((lead(Deaths_Cumulative) - lag(Deaths_Cumulative)) / 2),
    #                                Deaths_Cumulative_Imputed)
    #       ) |>
  ...
  stopifnot(
  DSHS_vitals_long |>
  # select(Cases_Cumulative, Cases_Cumulative_Imputed) |>
  filter(missing_day == 0) |>
  filter(Cases_Cumulative != Cases_Cumulative_Imputed & Date != '2022-08-25') |>
  nrow() == 0
  )
    ...
    DSHS_vitals_long = DSHS_vitals_long_impute_prep |> ungroup() |>  select(-any_of(c('impute_group', 'missing_day')))
  }

  DSHS_vitals_long |> filter(County == 'Bexar') |> filter(Date > '2022-08-20') |> View()


  # mutate(Cases_Daily_Imputed = ifelse(is.na(Cases_Daily_Imputed), Cases_Daily, Cases_Daily_Imputed)) |>
  # mutate(Deaths_Daily_Imputed = ifelse(is.na(Deaths_Daily_Imputed), Deaths_Daily, Deaths_Daily_Imputed)) |>
```
...
merged_dshs = DSHS_vitals_long |>
  ...
  mutate(across(c(Cases_Cumulative, Cases_Daily, Deaths_Cumulative, Deaths_Daily), ~ifelse(Date %in% MISSING_VITALS_DATES, NA, .)))
```
...
#   # MISSING_VITALS_DATES = seq.Date(from=as.Date('2021-12-10'), to=as.Date('2021-12-31'), by = 'week')
#   # for (m_date in as.character(MISSING_VITALS_DATES)) { 
