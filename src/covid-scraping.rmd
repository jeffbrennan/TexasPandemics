...
DSHS_schools_clean = DSHS_schools_raw[first_row:last_row, ] %>%
  ...
  # left_join(., county_isd_wide, by = 'LEA') %>%
...
#  
# 
# district_wide = read.csv('original-sources/helpers/county_isd_wide.csv')
# str_count(district_wide$County, ',') %>% max()
# district_long = district_wide %>%
#   separate(., County, paste("County", c(1:9), sep="_"), sep=",", extra="drop") %>%
#   reshape2::melt(id = c('District', 'LEA')) %>%
#   na.omit() %>%
#   dplyr::select(-variable) %>%
#   rename(County = value)
# # 
# write.csv(district_long, 'original-sources/helpers/county_isd_long.csv', row.names = FALSE)
# 
...
# 11/1 - 11/18 [added 0 to 11/19 and 11/20 as that data is not available yet]
montgomery_new_cases = c(32, 61, 78, 110, 98, 124, 78, 77, 144, 97, 146, 133, 124, 103, 90, 109, 33, 3, 0, 0)
montgomery_case_dates = seq(as.Date('2020-11-01'), as.Date('2020-11-20'), by = 'day')
...
county_classifications = read_xlsx('original-sources/helpers/county_classifications.xlsx', sheet = 1)[1:254, c(1, 5, 8)]
...
tsa_long_complete = merge(tsa_long_complete, DSHS_tsa_names, by = 'TSA') %>% distinct()
```
...
DSHS_state_day = data.frame(Antigen_Tests_Total = DSHS_state[[6]][1,2],
