...
if (format(date_out, '%A') == 'Monday') {
  ...
  age_archive = rbindlist(lapply(demo_archive %>% .[str_detect(., 'age')], read_csv))
  race_archive = rbindlist(lapply(demo_archive %>% .[str_detect(., 'race')], read_csv))
  
  
  vax_age = rbindlist(lapply(list.files(glue('{vax_dir}demo_age/'), full.names = TRUE),
                                read.csv, fileEncoding="UTF-16LE", sep = '\t'),
                         fill = TRUE) %>%
    dplyr::select(-'X.1') %>%
  ...
  vax_race = rbindlist(lapply(list.files(glue('{vax_dir}/demo_race/'), full.names = TRUE),
                               Read_Race),
                         fill = TRUE) %>%
  ...
  vax_out = vax_allocation %>% full_join(vax_admin, by = c('Date', 'County')) %>% left_join(vax_pop_counts, by = 'County')
