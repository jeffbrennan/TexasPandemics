...
# TODO add population5
# TODO fix gender pop 5 total = gender total
test = county_demo_agesex %>%
              filter(`Age Group` %in% c('5-11 years', '12-15 years', '16+')) %>%
              group_by(Gender) %>%
              summarize(Population_5 = sum(Population_Total))
...
hosp_1 = DSHS_hosp_clean(DSHS_tsa_hosp[['COVID-19 Hospitalizations']], 'Hospitalizations_Total')  # [1]
hosp_2 = DSHS_hosp_clean(DSHS_tsa_hosp[['Adult COVID-19 General']], 'Hospitalizations_General')
hosp_3 = DSHS_hosp_clean(DSHS_tsa_hosp[['Adult COVID-19 ICU']], 'Hospitalizations_ICU')
hosp_4 = DSHS_hosp_clean(DSHS_tsa_hosp[['Available Ventilators']], 'Ventilators_Available')
hosp_5 = DSHS_hosp_clean(DSHS_tsa_hosp[['Pediatric COVID-19']], 'Hospitalizations_Pediatric')


hosp_cap1 = DSHS_hosp_clean(DSHS_tsa_hosp[['Total Available Beds']], 'Beds_Available_Total')
hosp_cap2 = DSHS_hosp_clean(DSHS_tsa_hosp[['Adult ICU Beds Available']], 'Beds_Available_ICU')
hosp_cap3 = DSHS_hosp_clean(DSHS_tsa_hosp[['Total Occupied Beds']], 'Beds_Occupied_Total')
hosp_cap4 = DSHS_hosp_clean(DSHS_tsa_hosp[['ICU Beds Occupied']], 'Beds_Occupied_ICU')
hosp_cap5 = DSHS_hosp_clean(DSHS_tsa_hosp[['Pediatric ICU Beds Available']], 'Pediatric_Beds_Available_ICU')
...
#                        TSA = c("A"),
#                   TSA_Name = c("Amarillo"),
#               TSA_Combined = c("A - Amarillo"),
#            Population_DSHS = c(440127L),
#      Ventilators_Available = c(NA),
#     Hospitalizations_Total = c(17L),
#   Hospitalizations_General = c(417L),
#       Hospitalizations_ICU = c(613L),
#       Beds_Available_Total = c(4L),
#         Beds_Available_ICU = c(1030L),
#        Beds_Occupied_Total = c(NA),
#          Beds_Occupied_ICU = c(77L)
# )
# 
# 8/29/2021	Q	Houston	Q - Houston	6688587	985	3423	12450	1475	814	14636	1817	72
...
merged_tsa = Reduce(function(x, y) merge(x, y, by = c('Date', 'TSA'), all = TRUE),
                    list(DSHS_tsa,
                         hosp_1, hosp_2, hosp_3, hosp_4, hosp_5,
                         hosp_cap1, hosp_cap2, hosp_cap3, hosp_cap4, hosp_cap5))
...
merged_tsa = merged_tsa %>% 
  mutate_at(vars(Beds_Available_Total, Beds_Available_ICU, Ventilators_Available,
                 ...
                 Hospitalizations_Pediatric, Pediatric_Beds_Available_ICU),
            funs(as.character)) %>%
  mutate_at(vars(Beds_Available_Total, Beds_Available_ICU, Ventilators_Available,
                 Beds_Occupied_Total, Beds_Occupied_ICU, Hospitalizations_Total,
                 Hospitalizations_General, Hospitalizations_ICU,
...
# # October 1st - june30 2021
# # july - december 11 2021
# # december 12 - current third wave
...
# hospitalizations result in ratios >1 because most people have stays longer than 1 day (double counted)
# using deaths instead
wave_comparison = fread('tableau/county.csv') %>%
  mutate(Date = as.Date(Date)) %>% 
  group_by(Date) %>% 
  ...
  #                       Hospitalizations_ICU = sum(Hospitalizations_ICU, na.rm = TRUE)),
  #           by = c('Date' = 'Date')) %>% 
  left_join(waves, by = c('Date' = 'Date')) %>% 
  filter(!is.na(Wave)) %>%
  group_by(Wave) %>%
  summarize(Cases_Total = sum(Cases_Daily, na.rm = TRUE),
            Deaths_Total = sum(Deaths_Daily, na.rm = TRUE)) %>%
  mutate(Death_Ratio = Deaths_Total / Cases_Total)
    # summarize(Hospitalizations_Total = sum(Hospitalizations_Total, na.rm = TRUE),
  #           Hospitalizations_ICU = sum(Hospitalizations_ICU, na.rm = TRUE),
  #           Cases_Total = sum(Cases_Daily, na.rm = TRUE)) %>% 
  # mutate(Hospitalizations_Total_Ratio = Hospitalizations_Total / Cases_Total) %>% 
  # mutate(Hospitalizations_ICU_Ratio = Hospitalizations_ICU / Cases_Total)
  # 
...
if (date_out %in% demo_releases) {
  ...
  monthly_update_dates = str_extract(all_demo_files, '\\d{4}-\\d{2}-\\d{2}') %>%
    ...
    .[which(. >= as.Date('2022-01-14'))]
                             # function(x) all_demo_files[which(str_detect(all_demo_files, as.character(x)))])
