...
file_url = 'https://beta.healthdata.gov/api/views/gqxm-d9w9/files/ae46b5fc-8e03-4e7d-95a0-6f9205720abb?download=true&filename=Community_Profile_Report_20210205_Public.xlsx'
...
```{r}
vax_dir = 'original-sources/historical/vax-dashboard/temp/'
...
vax_race = rbindlist(lapply(list.files(glue('{vax_dir}/demo_race/'), full.names = TRUE),
                             Read_Race),
                       fill = TRUE) %>% 
  setNames(c('Race', 'Fully_Vaccinated', 'dump1', 'dump2', 'County')) %>% 
  mutate(Date = max(vax_admin$Date)) %>%
