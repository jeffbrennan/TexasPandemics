...
if (format(date_out, '%A') == 'Tuesday') {
  vax_dir = 'original-sources/historical/vax-dashboard/'
  vax_allocation = rbindlist(lapply(list.files(glue('{vax_dir}allocation/'), full.names = TRUE),
                                read.csv, fileEncoding="UTF-16LE", sep = '\t'),
                         fill = TRUE) %>%
    filter(X != 'DosesAllocatedWindow along Dose Number') %>%
    reshape2::melt(id = c('Allocation.Week.Range', 'CountyNameDisplay', 'Dose.Number')) %>%
  ...
  vax_admin = rbindlist(lapply(list.files(glue('{vax_dir}admin/'), full.names = TRUE),
                                read.csv, fileEncoding="UTF-16LE", sep = '\t'),
                         fill = TRUE) %>%
...
## FULL
  ...
  age_archive_full = rbindlist(lapply(demo_archive %>% .[str_detect(., 'age_full')], read_csv))
  race_archive_full = rbindlist(lapply(demo_archive %>% .[str_detect(., 'race_full')], read_csv))
  
  vax_age_full = rbindlist(lapply(list.files(glue('{vax_dir}demo_age/'), full.names = TRUE),
                                  read.csv, fileEncoding="UTF-16LE", sep = '\t'),
                           fill = TRUE) %>%
    dplyr::select(-'X.1') %>%
    reshape2::melt(id = c('X', 'X.2')) %>%
  ...
  vax_race_full = rbindlist(lapply(list.files(glue('{vax_dir}/demo_race/'), full.names = TRUE),
                               Read_Race),
                         fill = TRUE) %>%
    setNames(c('Race', 'Fully_Vaccinated', 'dump1', 'County')) %>%
    mutate(Date = max(vax_admin$Date)) %>%
    dplyr::select(Date, County, Race, Fully_Vaccinated, -dump1) %>% 
    rbind(race_archive) %>% 
    mutate(Date = as.Date(Date)) %>%
    arrange(Date, County)
...
# PARTIAL
  age_archive_partial = rbindlist(lapply(demo_archive %>% .[str_detect(., 'age_partial')], read_csv))
  race_archive_partial = rbindlist(lapply(demo_archive %>% .[str_detect(., 'race_partiaL')], read_csv))
  
  vax_age_partial = rbindlist(lapply(list.files(glue('{vax_dir}demo_age/'), full.names = TRUE),
                                  read.csv, fileEncoding="UTF-16LE", sep = '\t'),
                           fill = TRUE) %>%
    dplyr::select(-'X.1') %>%
    reshape2::melt(id = c('X', 'X.2')) %>%
    ...
    mutate(County = gsub(' County', '', County)) %>%
    dplyr::select(Date, County, everything()) %>% 
    rbind(age_archive_partial) %>% 
    mutate(Date = as.Date(Date)) %>%
    arrange(Date, County)


  vax_race_partial = rbindlist(lapply(list.files(glue('{vax_dir}/demo_race/'), full.names = TRUE),
                               Read_Race),
                         fill = TRUE) %>%
    setNames(c('Race', 'All_Vaccinated', 'dump1', 'County')) %>%
    mutate(Date = max(vax_admin$Date)) %>%
    dplyr::select(Date, County, Race, All_Vaccinated, -dump1) %>% 
    rbind(race_archive_partial) %>% 
    mutate(Date = as.Date(Date)) %>%
    arrange(Date, County)
 
  vax_age = vax_age_full %>% left_join(vax_age_partial, by = c('County', 'Date'))
  vax_race = vax_race_full %>% left_join(vax_race_partial, by = c('County', 'Date'))
  ...
  vax_out = vax_allocation %>% 
    ...
    full_join(vax_age_full %>% 
                group_by(Age, Gender) %>%
                mutate(All_Vaccinated = sum(All_Vaccinated)) %>% 
                dplyr::select(Date, County, All_Vaccinated))
  ...
  write.csv(vax_age, glue('{vax_dir}archive/{max(vax_admin$Date)}_demographics_vax_age.csv'), row.names = FALSE)
  write.csv(vax_race, 'tableau/demographics_vax_race.csv', row.names = FALSE)
  write.csv(vax_race, glue('{vax_dir}archive/{max(vax_admin$Date)}_demographics_vax_race.csv'), row.names = FALSE)
