...
### setup
```{r}
VITALS_URL = 'https://www.dshs.texas.gov/sites/default/files/chs/data/CaseCountData.xlsx'
temp = tempfile()
curl::curl_download(VITALS_URL, 'temp.xlsx', mode = 'wb')

VITALS_DATE = readxl::read_xlsx('temp.xlsx', sheet = 'Case and Fatalities_ALL') %>%
  names() %>%
  .[1] %>%
...
if (IMPUTED_CHECK) {
  ...
  DSHS_vitals_long = DSHS_vitals_long_impute_prep |>
    ungroup() |>
    select(-any_of(c('impute_group', 'missing_day')))
}

# DSHS_vitals_long |>
#   filter(County == 'Bexar') |>
#   filter(Date > '2022-08-20') |>
#   View()
...
```{r}
vaccine_county_dshs_url_base = 'https://www.dshs.texas.gov/sites/default/files/LIDS-Immunize-COVID19/COVID%20Dashboard/COVID-19%20Vaccine%20Data%20by%20County'

if (format(date_out, '%A') == 'Wednesday') {

  vaccine_county_dshs_url = str_c(vaccine_county_dshs_url_base, format(date_out, '%m.%d.%y'), '.xlsx')


  curl::curl_download(vaccine_county_dshs_url,
                      mode     = 'wb',
...
```{r}
# updates from DSHS ceased 12/01
