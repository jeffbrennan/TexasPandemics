...
DSHS_vitals_long_raw = readxl::read_xlsx(NEWEST_CASE_FILE, sheet = 'Case and Fatalities_ALL', skip = 1) |> 
...
DSHS_vitals_long_impute_prep = DSHS_vitals_long_raw |> 
  ...
  mutate(Cases_Cumulative = ifelse(Date < VITALS_DATE - days(5) & is.na(Cases_Cumulative),
                                   Cases_Cumulative_Imputed,
                                   Cases_Cumulative)) |> 
  mutate(Deaths_Cumulative = ifelse(Date < VITALS_DATE - days(5) & is.na(Deaths_Cumulative),
  ...
  mutate(missing_day = cumsum(is.na(Cases_Cumulative) & Date > (VITALS_DATE - days(5)))) |> 
  mutate(missing_day = ifelse(missing_day == lag(missing_day) | missing_day == 0 | row_number() == 1, 0, missing_day)) |> 
  mutate(impute_group = is.na(Cases_Cumulative_Imputed) & (missing_day != 0 | lead(missing_day) != 0 | lag(missing_day) != 0))
...
DSHS_vitals_long_imputed = DSHS_vitals_long_impute_prep |> 
  ...
  ungroup() |> 
  select(County, Date, contains('Imputed'))
...
DSHS_vitals_long = 
  ...
  group_by(County) |> 
  mutate(Cases_Cumulative_Imputed = ifelse(Date == '2022-08-25',
                                           Cases_Cumulative + ((lead(Cases_Cumulative) - lag(Cases_Cumulative)) / 2),
                                           Cases_Cumulative_Imputed
                                           )
        ) |> 
  mutate(Deaths_Cumulative_Imputed = ifelse(Date == '2022-08-25',
...
merged_dshs = DSHS_vitals_long |>
  ...
  mutate(across(c(Cases_Cumulative, Cases_Daily, Deaths_Cumulative, Deaths_Daily), ~ifelse(Date == '2022-08-25', NA, .)))
```
