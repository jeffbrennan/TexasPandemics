...
current_school_dates = seq(as.Date('2021-08-15'), by = 'week', length = 52)
...
DSHS_schools <- glue('{base_url}{format(school_date + 2, "%Y%m%d")}v324.xls') %>%
  Download_Temp(.) %>%
  read_excel(sheet = 1) %>%
  data.frame() %>% 
  Clean_DSHS_Schools(.) %>% 
...
Data covering 2020-11-15 - 2021-08-01

```{r}
# historical pull - saved for reference
# historical_schools =
#   Download_Temp('https://dshs.texas.gov/chs/data/tea/Public-School-Data-Files-2020_2021-School-Year.xls') %>%
#   read_excel_allsheets(skip_option = 5) %>% 
#   .[-1]
# 
# # excludes weeks between 9/20 and 11/15 due to major file differences
# # 2 months of more specific school level data which is unusable for longitudinal trending
# school_dates = seq(as.Date('2020-11-22'), as.Date('2021-08-01'), by = 'week') %>% rev()
# 
# 
# 

# cleaned_schools = lapply(names(historical_schools), Clean_Historical_Schools) %>% 
#   rbindlist(fill = TRUE) %>% 
#   arrange(District, Date)
# 
# 
# write_csv(cleaned_schools,
#           file = './original-sources/historical/dshs-schools/historical_archive.csv')
```
...
hosp_1 = DSHS_hosp_clean(DSHS_tsa_hosp[['COVID-19 Hospitalizations']], 'Hospitalizations_Total')
...
hosp_cap2 = DSHS_hosp_clean(DSHS_tsa_hosp[['ICU Beds Available']], 'Beds_Available_ICU')
...
```{r}
# high cases 7/6
high_cases = merged_county %>% 
  filter(County %in% c('Presidio', 'Hudspeth', 'Starr', 'Maverick', 'Uvalde', 'Harris')) %>% 
  filter(Date >= as.Date('2021-06-15')) %>% 
  select(Date, County, Cases_Cumulative, Cases_Daily, Population_DSHS) %>%
  arrange(County, Date) %>% 
  mutate(Cases_100K = (Cases_Daily / Population_DSHS) * 100000)
  
ggplot(high_cases, aes(x = Date, y = Cases_100K)) + 
  # geom_point() + 
  geom_line() + 
  geom_label(aes(label = round(Cases_100K, 0))) +
  facet_wrap(~County) + 
  theme_pubr()


high_cases %>% filter(County == 'Presidio')
