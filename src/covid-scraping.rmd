...
#  convert hispanic asian -> hispanic & non hispanic asian -> asian
race_group_df = data.frame(race = 
                           ...
                           race_group = 
                             c(NA, NA, NA, NA, 'Hispanic', NA,
                               NA, NA, NA, NA, NA , NA,
                               NA, NA, NA, NA, NA, NA,
                               NA, NA, NA, 'Asian', NA, 'Black', NA,
                              'Other', NA, 'Other', NA, 'Other', 'White',
                              NA, NA, NA, NA))

county_demo_race = read_csv('original-sources/helpers/demographics/county/county_pop_race.csv') %>% 
...
# DSHS swapped counts of full and partial vaccinations on 3/12. Below is temp dirty fix 
# TODO: clean up code
vaccinations_312 = 
...
county_vaccinations = county_vaccinations %>% 
...
# county_vaccinations %>% 
#   group_by(Date) %>% 
#   summarize(Full_Vax = sum(Fully_Vaccinated, na.rm = TRUE)) %>%
#   ggplot(aes(x = Date, y = Full_Vax)) + 
#   geom_line() + 
#   geom_point() + 
#   theme_pubr()
# 

write_csv(county_vaccinations, 'tableau/sandbox/county_daily_vaccine.csv')
```
...
#   rbindlist(
#   lapply(
#     lapply(all_dashboard_files, `[[`, 'By Age, Gender, Race') %>% discard(is.null),
#            function(x) return(x %>%
#                                 setNames(slice(., 1)) %>%
#                                 slice(2:nrow(.)) %>%
#                                 rename('Date' = ncol(.)))
...
state_demo %>% summarize(sum(State_Gender_Total, na.rm = TRUE))
...
```{r}
read_csv('tableau/sandbox/county_daily_vaccine.csv') %>% 
  filter(County %in% c('Jefferson', 'Liberty', 'Orange', 'Jasper', 'Tyler', 'Polk')) %>%
  select(Date, County, Doses_Administered, Doses_Allocated) %>% 
  # reshape2::melt(id.vars = c('Date', 'County'))
  ggplot(aes(x = Date)) +
  geom_point(aes(y = Doses_Administered, color = 'Doses Administered')) + 
  geom_point(aes(y = Doses_Allocated, color = 'Doses Allocated')) + 
  geom_line(aes(y = Doses_Administered, color = 'Doses Administered')) + 
  geom_line(aes(y = Doses_Allocated, color = 'Doses Allocated')) +
  scale_color_manual('', values = c(`Doses Administered` = 'dodgerblue',
                                `Doses Allocated` = 'orange')) +
...
#   rbindlist(
#   lapply(
#     lapply(all_dashboard_files, `[[`,  'By County, Race') %>% discard(is.null),
#            function(x) return(x %>%
#                                 setNames(slice(., 1)) %>%
#                                 slice(2:nrow(.)) %>%
#                                 rename('Date' = ncol(.)))
...
#   rbindlist(
#   lapply(
#     lapply(all_dashboard_files, `[[`,  'By County, Age') %>% discard(is.null),
#            function(x) return(x %>%
#                                 setNames(slice(., 1)) %>%
#                                 slice(2:nrow(.)) %>%
#                                 rename('Date' = ncol(.)))
...
vaccination_quartile = county_vaccinations %>%
  ...
  select(Date, County, Vaccination_Rate, Population_16) %>%
  na.omit() %>% 
  group_by(Date) %>%
...
vax_cases = DSHS_new_cases_long %>% 
  left_join(county_vaccinations %>%
              ...
              # mutate(Vaccination_Rate = Fully_Vaccinated / Population_16) %>%
              left_join(vaccination_quartile, by = c('County', 'Date')) %>%
...
# individual counties colored by group
...
p1 = vax_cases %>%
  ...
  na.omit() %>%
  ggplot(aes(x = Date, y = Cases_100K_14Day_MA, color = County, fill = County)) +
  geom_smooth(alpha = 0.5) +
  labs(y = 'Cases Per 100K (14 Day MA)',
...
p1 = vax_hospitalizations %>%
  ...
  # mutate(Vax_Category = ifelse(County %in% top_5_counties, 'Top_5', 'Bottom_5')) %>%
  na.omit() %>%
  ggplot(aes(x = Date, y = Hosp_100K_14Day_MA, color = TSA, fill = TSA)) +
  geom_smooth(alpha = 0.5) +
