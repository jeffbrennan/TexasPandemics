...
```{r}
# TSA levels
tsa_url = 'https://dshs.state.tx.us/coronavirus/TexasCOVID-19HospitalizationsOverTimebyTSA.xlsx'
temp = tempfile()
...
Clean_Vitals = function(df, data_type) { 
  ...
  df_out = df %>%
    ...
    filter(!Date %in% c('Total', 'Unknown Date', 'Unknown.Date')) %>% 
...
case_url = 'http://dshs.state.tx.us/coronavirus/TexasCOVID19DailyCountyCaseCountData.xlsx'
# temp = tempfile()
curl::curl_download(case_url,
                    'original-sources/historical/temp/dshs_cases.xlsx',
                    mode = 'wb') 
DSHS_cases = read_excel_allsheets('original-sources/historical/temp/dshs_cases.xlsx',
                                  skip_option = 2)
...
DSHS_cases_long =  lapply(DSHS_cases, function(x) Clean_Vitals(x, 'Cases_Cumulative')) %>% 
  ...
  filter(Cases_Cumulative == max(Cases_Cumulative, na.rm = TRUE)) |> 
  ungroup()
```
...
```{r}
death_url = 'https://dshs.state.tx.us/coronavirus/TexasCOVID19DailyCountyFatalityCountData.xlsx'
# temp = tempfile()
curl::curl_download(death_url,
                    'original-sources/historical/temp/dshs_deaths.xlsx',
                    mode = 'wb') 

DSHS_deaths = read_excel_allsheets('original-sources/historical/temp/dshs_deaths.xlsx', 
                                   skip_option = 2)
...
# NEW
test_url = 'https://dshs.state.tx.us/coronavirus/TexasCOVID-19CumulativeTestsbyCounty.xlsx'
# temp = tempfile()
curl::curl_download(test_url,
                    'original-sources/historical/temp/dshs_test.xlsx',
                    mode = 'wb')

DSHS_tests = read_excel_allsheets('original-sources/historical/temp/dshs_test.xlsx',
                                  skip_option = 1)
...
new_tests = data.frame(DSHS_tests[[3]])
...
new_case_url ='https://dshs.state.tx.us/coronavirus/TexasCOVID-19NewCasesOverTimebyCounty.xlsx'
# temp = tempfile()
curl::curl_download(new_case_url,
                    'original-sources/historical/temp/dshs_new_cases.xlsx',
                    mode = 'wb') 
DSHS_new_cases = read_excel_allsheets('original-sources/historical/temp/dshs_new_cases.xlsx',
                                      skip_option = 2)
...
NEW_CASES_MAX_DATE = max(DSHS_new_cases_long$Date)
CUMULATIVE_CASES_MAX_DATE = max(DSHS_cases_long$Date)

if (NEW_CASES_MAX_DATE < CUMULATIVE_CASES_MAX_DATE) { 
  DSHS_new_cases_long = DSHS_cases_long |>
    arrange(Date) |> 
    group_by(County) |> 
    ...
    ungroup() |> 
    select(-Cases_Cumulative, -Cases_Cumulative_NA)
}
```
...
# current
new_case_url = 'https://dshs.state.tx.us/coronavirus/TexasCOVID-19ActiveCaseDatabyCounty.xlsx'
temp = tempfile()
...
new_active_cases_long = reshape::melt(new_active_cases, id = 'County') %>% 
  ...
  group_by(Date) %>%
  mutate(Date = str_extract_all(Date, '\\d+')[[1]]) %>% 
...
fwrite(merged_county_out %>% arrange(County, Date), 'tableau/county.csv')
```
