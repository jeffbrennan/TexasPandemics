...
tryCatch({
  ...
  temp = tempfile()
  curl::curl_download(school_url, temp, mode = 'wb')
...
temp = tempfile()
curl::curl_download(file_url, temp, mode = 'wb')
...
# variant_df = read_html('http://www.dshs.texas.gov/news/updates.shtm') %>%
#   html_table() %>%
#   .[[which(str_detect(., 'ALPHA'))]] %>%
#   filter(TSA != 'Grand Total') %>% 
#   select(-Total) %>% 
...
write_csv(x = all_variants %>% filter(Date %in% update_dates), 
