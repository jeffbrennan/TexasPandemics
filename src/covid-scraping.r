...
county_names_vec = county_classifications$County %>% unique()
...
#
# # combine cpr archive
# tpr_results_cleaned = rbindlist(lapply(list.files('original-sources/historical/cpr/', full.names = TRUE), read.csv)) %>%
#   rename(TPR = TPR_CPR) %>%
#   mutate(Date = as.Date(Date)) %>%
