...
def parse_scraping_output(f):
    # TODO: refactor    
    if ('Output created: diagnostics/covid-scraping.html' in f):
        scrape = [':heavy_check_mark:', '']

        if ('Output created: diagnostics/statistical-analysis.html' in f): 
            stats = [':heavy_check_mark:', '']

            if ('Output created: diagnostics/diagnostics.html' in f):
...
if __name__ == '__main__':
	...
	scraping_output = open("src/scrape-sources.Rout", "r").read()
