...
county_TPR = county_TPR_padded %>% filter(!(Date %in% district_dates$Date))
county_TPR_sd = county_TPR_padded %>%  filter(Date %in% district_dates$Date)
...
```{r}
PCT_County_df = rbindlist(lapply(unique(TPR_county$County), function(x) create.case.test('County', TPR_county, x)))
```

### TSA

```{r pct change}
PCT_TSA_df = rbindlist(lapply(unique(TPR_TSA$TSA), function(x) create.case.test('TSA', TPR_TSA, x)))
```

### PHR

```{r}
PCT_PHR_df = rbindlist(lapply(unique(TPR_PHR$PHR), function(x) create.case.test('PHR', TPR_PHR, x)))
```

### Metro

```{r}
PCT_Metro_df = rbindlist(lapply(unique(TPR_Metro$Metro), function(x) create.case.test('Metro', TPR_Metro, x)))
```

### State

```{r}
PCT_State_df = create.case.test('State', TPR_State, 'Texas')
```
