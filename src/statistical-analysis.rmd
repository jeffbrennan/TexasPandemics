...
# county_TPR = read.csv('tableau/county_TPR.csv') %>%
#   dplyr::select(-Rt) %>%
#   mutate(Date = as.Date(county_TPR$Date, '%m/%d/%y')) %>%
#   left_join(., RT_County_df[, c('County', 'Date', 'Rt')], by = c('County', 'Date'))

# county_TPR_match = county_TPR %>% 
#   mutate(Date = Date-3) %>% 
#   dplyr::select(County, Date, TPR_CMS, Cases_100K_7Day_MA, Rt)
# 
# 

# district_link = readxl::read_xlsx('tableau/district_school_reopening.xlsx', sheet=2)
# district_match = district %>% 
#   left_join(., district_link, by = c('District', 'LEA')) %>% 
#   left_join(., county_TPR_match, by = c('County', 'Date')) %>% 
#   select()
# 
# 
# write.csv(county_TPR, 'tableau/county_TPR.csv', row.names = FALSE)
# writexl::write_xlsx(list(district_match, district_link), 'tableau/district_school_reopening_merge.xlsx', format_headers = FALSE)
# 

# # district_dates = data.frame('Date' = unique(district$Date))
...
county_TPR_padded = 
  read.csv('tableau/county_TPR.csv') %>%
  mutate(Date = as.Date(Date)) %>%
  full_join(., district_dates, by = c('County', 'Date')) %>% 
  left_join(., RT_County_df[, c('County', 'Date', 'Rt')], by = c('County', 'Date')) %>%
  group_by(County) %>% 
  arrange(County, Date) %>%
  tidyr::fill(TPR_CMS, .direction = 'up') %>%
  tidyr::fill(Tests, .direction = 'up') %>% 
  tidyr::fill(Cases_100K_7Day_MA, .direction = 'up') %>% 
  tidyr::fill(Rt, .direction = 'up')

write.csv(county_TPR_padded, 'tableau/county_TPR.csv', row.names = FALSE)

# 
# district_combined = district %>% left_join(., district_link, by = c('District', 'LEA')) %>% 
#   mutate(Date = Date + 3)
# 
# county_TPR %>% 
#   left_join(., district_combined, by = c('Date', 'County'))
