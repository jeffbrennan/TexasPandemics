...
covid.rt <- function(mydata, threshold) {
  ...
  recent_case_avg = mydata %>% filter(Date > seq(Sys.Date(), length = 2, by = "-1 month")[2]) %>%
...
case_quant = county.daily %>% filter(Date >= as.Date(Sys.Date() - as.difftime(3, unit = 'weeks'))) %>%
  group_by(County) %>%
  mutate(mean_cases = mean(Cases_Daily, na.rm = TRUE)) %>%
  dplyr::select(mean_cases) %>%
  ungroup() %>%
  summarize(case_quant = quantile(mean_cases, c(0.4, 0.5, 0.6, 0.7, 0.8))[4]) %>% 
  unlist()
```
