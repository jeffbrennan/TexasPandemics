...
```{r}
colnames(county.rt.df)[1] = 'Level_Name'
colnames(metro.rt.df)[1] = 'Level_Name'
colnames(TSA.rt.df)[1] = 'Level_Name'

state.rt.df$Level_Name = 'Texas'

combined.rt.df = rbind(county.rt.df, metro.rt.df, TSA.rt.df, state.rt.df)
write.csv(combined.rt.df, 'statistical-output/rt/combined_rt.csv', row.names = FALSE)
```
...
write.csv(combined.arima.df, 'statistical-output/time-series/combined_timeseries.csv', row.names = FALSE)
```
...
```{r}
colnames(county_current_ratio_dat)[1] = 'Level_Name'
colnames(metro_current_ratio_dat)[1] = 'Level_Name'
colnames(tsa_current_ratio_dat)[1] = 'Level_Name'

state_current_ratio_dat$Level_Name = 'Texas'
...
write.csv(combined_current_ratio_dat, 'statistical-output/standard-stats/case-ratios/combined_case_ratio.csv', row.names = FALSE)
```
...
```{r}
colnames(county.case.test.df)[1] = 'Level_Name'
colnames(metro.case.test.df)[1] = 'Level_Name'
colnames(tsa.case.test.df)[1] = 'Level_Name'

state.case.test.df$Level_Name = 'Texas'

combined.case.test.df = rbind(county.case.test.df, metro.case.test.df, tsa.case.test.df, state.case.test.df)
write.csv(combined.case.test.df, 'statistical-output/standard-stats/pct-change/combined_pct_change.csv', row.names = FALSE)
```
