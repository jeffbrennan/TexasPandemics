...
#select the 9 counties of interest
county.focus<-county.daily%>%filter(County=="Austin"|
                                      County=="Brazoria"|
                                      County=="Chambers"|
                                      County=="Fort Bend"|
                                      County=="Galveston"|
                                      ...
                                      County=="Montgomery"|
                                      County=="Waller")
#drop county variable and aggregate other variables by date
library(reshape2)
county.focus<-county.focus%>%dplyr::select(-County, -PHR, -PHR_Name)
county.focus.melt<-melt(county.focus, id="Date")
county.focus.cast<-dcast(county.focus.melt, Date~variable, sum)

# run covid.rt function on count.focus.cast dataframe
county.focus.rt.output<-covid.rt(county.focus.cast, threshold=case_quant) %>% na.omit()
...
```
## TMC RT ESTIMATS: SCHOOL DISTRICT COUNTIES
```{r TMC RT ESTIMATS: SCHOOL DISTRICT COUNTIES}
#select the counties associated with school districts of interest
school.focus<-county.daily%>%filter(County=="Brazoria"|
                                      County=="Galveston"|
                                      County=="Harris")%>%dplyr::select(-PHR, -PHR_Name)
...
#write csv of estimates to rt_TMC Path
write.csv(school.county.rt.df, paste(rt_TMC_path,"/nine_county_rt_estimates.csv",sep=""), row.names = FALSE)
...
harris.schools.plot<-school.rt.plot(harris.schools, "Pasadena, La Porte, Deer Park, Channel View, Galena Park, Sheldon, Crosby Goose Creek, n/ Humble, Aldine, Spring, Klein, Cyrrpress-Fairbanks, Katy, Spring Branch, Alief and Houston ISD")
harris.schools.plot
ggsave( paste(rt_TMC_path,"/harris_schools_rt_plot.png",sep=""), width=20, height=10, unit="cm")
...
ggsave( paste(rt_TMC_path,"/galveston_schools_rt_plot.png",sep=""), width=20, height=10, unit="cm")
...
ggsave( paste(rt_TMC_path,"/brazoria_schools_rt_plot.png",sep=""), width=20, height=10, unit="cm")
```
