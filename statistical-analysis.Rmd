...
Clean_Data = function(df, level_type) { 
  if (level_type == 'State') {
    clean_df  = df %>% 
      dplyr::select(Date,
                    Cases_Daily, Cases_100K_7Day_MA,
                    Tests_Daily, TPR_CMS,
                    Population_DSHS) %>%
      group_by(Date) %>% 
    ...
    clean_df = df %>% 
      dplyr::select(Date, !!as.name(level_type),
                    Cases_Daily, Cases_100K_7Day_MA,
                    Tests_Daily, TPR_CMS,
                    Population_DSHS) %>%
...
county_tpr = read.csv('tableau/county_TPR.csv') %>% 
  dplyr::select(County, Date, TPR_CMS, Cases_100K_7Day_MA) %>% 
  mutate(Date = as.Date(Date))
...
county = read.csv("tableau/county.csv") %>% 
  ...
  mutate(TPR_CMS = ifelse(Date < min(county_tpr$Date), NA, TPR_CMS),
         Cases_100K_7Day_MA = ifelse(Date < min(county_tpr$Date), NA, Cases_100K_7Day_MA)) %>%
  group_by(County) %>%
  tidyr::fill(TPR_CMS, .direction = "down") %>%
...
covid.rt = function(mydata, threshold) {
  ...
  if(!is.na(minrow) & recent_case_avg > threshold) {
    ...
    rt.DSHS.df = rt.df.extraction(rt.DSHS) %>% 
                    mutate(TPR_CMS = mydata$TPR_CMS,
                           Cases_100K_7Day_MA = mydata$Cases_100K_7Day_MA) %>% 
                    dplyr::select(Date, TPR_CMS, Cases_100K_7Day_MA, Rt, lower, upper)
    },
    error = function(e) {
    writeLines(paste0('Rt generation error (despite sufficient cases)', '\n'))
    rt.DSHS.df = data.frame(Date = as.Date(mydata$Date),
                             TPR_CMS = mydata$TPR_CMS,
                             Cases_100K_7Day_MA = mydata$Cases_100K_7Day_MA,
...
RT_County_df = rbindlist(county.rt.output, idcol = 'County')
```

## School Districts
```{r school district rt}
school_districts = read.csv('tableau/district_school_reopening.csv') %>% 
  dplyr::select(District, LEA, County) %>%
  mutate(District = paste0(District, ' (', LEA, ')')) %>%
  distinct() %>%
  dplyr::select(-LEA)

# exclude ISDs w/ 1 county listed [no ,c ] - can get estimate from RT_County_df
county_combos = school_districts %>% 
  dplyr::select(County) %>% 
  distinct() %>% 
  filter(stringr::str_detect(County, ',')) %>% 
  setNames(NULL) %>% unlist()

# split counties by comma, filter county daily with those counties, and pass back into rt function
Create_Combo_County_df = function(combo) { 
  combo_counties = stringr::str_split(combo, ',') %>% unlist()
  combo_df = County_df %>%
    filter(County %in% combo_counties) %>%
    mutate(County_Group = combo) %>% 
    group_by(Date, County_Group) %>% 
    summarize(Cases_Daily = sum(Cases_Daily),
              TPR_CMS = mean(TPR_CMS, na.rm = TRUE),
              Cases_100K_7Day_MA = sum(Cases_100K_7Day_MA),
              Population_DSHS = sum(Population_DSHS)) %>%
    ungroup()
  return(combo_df)
}

RT_Combo_Output = lapply(county_combos,
                                function(x) covid.rt(Create_Combo_County_df(x),
                                                     threshold = case_quant))
RT_Combo_df = rbindlist(RT_Combo_Output, idcol = TRUE) %>%
  mutate(.id = county_combos[.id]) %>% 
  rename(County = .id)

RT_District_Combo_df = school_districts %>% 
  dplyr::select(District, County) %>%
  filter(str_detect(County, ',')) %>%
  left_join(., RT_Combo_df, by = 'County')

RT_District_single_df = school_districts %>% 
  dplyr::select(District, County) %>%
  filter(!str_detect(County, ',')) %>%
  left_join(., RT_County_df, by = 'County')

RT_District_df = rbind(RT_District_Combo_df, RT_District_single_df) %>% 
  distinct() %>%
  dplyr::select(District, Date, everything(), -County) %>%
...
colnames(RT_District_df)[1] = 'Level'
...
RT_Combined_df = 
  rbind(RT_District_df, RT_County_df, RT_TSA_df,
        RT_PHR_df, RT_Metro_df, RT_State_df) %>% 
...
```{r}
PCT_County_df = rbindlist(lapply(unique(County_df$County), function(x) create.case.test('County', County_df, x)))
```

### TSA

```{r pct change}
PCT_TSA_df = rbindlist(lapply(unique(TSA_df$TSA), function(x) create.case.test('TSA', TSA_df, x)))
```

### PHR

```{r}
PCT_PHR_df = rbindlist(lapply(unique(PHR_df$PHR), function(x) create.case.test('PHR', PHR_df, x)))
```

### Metro

```{r}
PCT_Metro_df = rbindlist(lapply(unique(Metro_df$Metro), function(x) create.case.test('Metro', Metro_df, x)))
```

### State

```{r}
PCT_State_df = create.case.test('State', State_df, 'Texas')
```
