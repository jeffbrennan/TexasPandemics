...
rt.data.clean<-function(covid.data) {
  ...
  rt.cols <- c("County", "Date", "TSA_Name", "TSA",
...
rt.data.organize <- function(mycounty, mytsa){
  ...
  ### OUTPUT ###
  rt.df.out <- list(county=county.df, TSA=TSA.df, state=state.df, metro=metro.df)
  return(rt.df.out)
}
...
ts.data.clean<-function(covid.data) {
  ...
  ts_cols <- c("County", "Date", "TSA", "TSA_Name",
...
ts.data.organize<-function(mycounty, mytsa){
  ...
  ### OUTPUT ###
  ts.df.out <- list(county=county.df, TSA=TSA.df, state=state.df, metro=metro.df)
  return(ts.df.out)
}
...
covid.arima.forecast<-function(mydata, prediction.period = 10, mindate)
{
  ...
  if(max(mydata$Cases_Daily>=100, na.rm = TRUE))
  {
    ...
    my.timeseries <- ts(rollmean(mydata$Cases_Daily, k=7, na.pad = TRUE, align = 'right'))
    ...
    arima.out <- data.frame(Date = seq(mindate, maxdate + prediction.period, by = 'days'),
                            ...
                            Order_Seasonal_Autoreg = c(rep(sp, times = model.length)),
                            Order_Seasonal_Moving_Avg = c(rep(sq, times = model.length)),
                            Period = c(rep(period, times= model.length)),
                            Differencing = c(rep(d, times= model.length)),
                            Differencing_Seasonal = c(rep(sd, times= model.length)))
    ...
    arima.out <- data.frame(Date = seq(mindate, maxdate + prediction.period, by = 'days'),
                            Cases_Daily = c(mydata$Cases_Daily, rep(NA, times = prediction.period)),
                            ...
                            Order_Seasonal_Autoreg = c(rep(NA, times = model.length)),
                            Order_Seasonal_Moving_Avg = c(rep(NA, times = model.length)),
                            Period = c(rep(NA, times= model.length)),
                            Differencing = c(rep(NA, times= model.length)),
                            Differencing_Seasonal = c(rep(NA, times= model.length)))
    }
...
texas.arima.df.1 <- covid.arima.forecast(state.Daily, mindate = as.Date('2020-05-01'))
...
write.csv(texas.arima.df.1, 'statistical-output/time-series/state_timeseries_05_01.csv', row.names = FALSE)
write.csv(TSA.arima.df.1, 'statistical-output/time-series/tsa_timeseries_05_01.csv', row.names = FALSE)
write.csv(county.arima.df.1, 'statistical-output/time-series/county_timeseries_05_01.csv', row.names = FALSE)
write.csv(metro.arima.df.1, 'statistical-output/time-series/metro_timeseries_05_01.csv', row.names = FALSE)
...
combined.arima.df.1 <- rbind(county.arima.df.1, metro.arima.df.1, TSA.arima.df.1, texas.arima.df.1)
write.csv(combined.arima.df.1, 'statistical-output/time-series/stacked_timeseries_05_01.csv',
          row.names = FALSE)

write.csv(combined.arima.df.1, 'tableau/stacked_timeseries_05_01.csv',
          row.names = FALSE)
...
# output
# TODO: investigate select vs subset(...)[, 'Level_Name']
...
combined_current_ratio_dat <- rbind(county_current_ratio_dat, metro_current_ratio_dat,
                                   tsa_current_ratio_dat, state_current_ratio_dat)
