...
covid.rt <- function(mydata) {
  ...
  # get cases and dates as vectors
  ...
  ### R0 ESTIMATION ###
  # TODO: establish better criteria for sum of daily cases
  if(!is.na(minrow) & sum.daily.cases > 75) {
...
combined.rt.df <- rbind(county.rt.df, metro.rt.df, TSA.rt.df, state.rt.df, phr.rt.df)
...
ts.data.organize<-function(mycounty, mytsa, myphr){
  ...
  state.temp<-TSA.df %>%
    ...
    mutate(state_pop_DSHS = sum(Population_DSHS, na.rm = TRUE)) %>%
    dplyr::select(Date, state_Cases_Daily, state_Cases_Cumulative, state_pop_DSHS) %>%
    distinct()
...
covid.arima.forecast<-function(mydata, prediction.period = 10, mindate)
{
  ...
  if(max(mydata$Cases_Daily>=100, na.rm = TRUE))
  {
    ...
    # dev.off()
    # ggsave(paste0('statistical-output/time-series/diagnostics/',
    #               mydata$Level[1],'/', 'residuals_', mydata$Level_Name[1], '.png'),
    #        plot = arima.resid.diagnostics)
    # 
    ...
    # sp<- arima.fit$arma[3]         # seasonal autoregressive order 
    # sq<- arima.fit$arma[4]         # seasonal moving average order 
    # period<- arima.fit$arma[5]     # period of data (note: 1 is for non seasonal data)
    # sd<- arima.fit$arma[7]         # seasonal differencing order 
    ...
    arima.out <- data.frame(Date = seq(mindate, maxdate + prediction.period, by = 'days'),
      ...
      arima.plot <- ggplot(arima.out, aes(x=Date, y = Cases_Daily))+
                    ...
                    # geom_line(aes(x=as.POSIXct(Date), y=CI.Lower), color="grey", size=0.1) +
                    # geom_line(aes(x=as.POSIXct(Date),y=CI.Upper), color="grey", size=0.1)
...
# # Phase 3
# TSA.arima.output.2 <- nlme::gapply(TSA.Daily,
#                                    FUN = covid.arima.forecast,
#                                    groups = TSA.Daily$Level_Name,
#                                    mindate = as.Date('2020-06-03'))
# 
# TSA.arima.df.2 <- data.table::rbindlist(TSA.arima.output.2, idcol=TRUE)
# 
# colnames(TSA.arima.df.2)[1] <- 'TSA'
# TSA.arima.df.2 <- merge(TSA.arima.df.2, unique(TSA.daily[, c('TSA', 'TSA_Name')]), by = c('TSA'))
# 
# # combine TSA 
# TSA.arima.df.2$TSA <- paste0(TSA.arima.df.2$TSA, ' - ', TSA.arima.df.2$TSA_Name)
# TSA.arima.df.2$TSA_Name <- NULL
```
...
PHR.arima.output <- nlme::gapply(PHR.Daily,
                                 FUN = covid.arima.forecast,
                                 groups = PHR.Daily$PHR,
                                 mindate = as.Date('2020-03-04'))

PHR.arima.df <- data.table::rbindlist(PHR.arima.output, idcol=TRUE)

colnames(PHR.arima.df)[1] <- 'PHR'
PHR.arima.df <- merge(PHR.arima.df, unique(PHR.Daily[, c('PHR', 'PHR_Name')]), by = c('PHR'))

# combine PHR 
PHR.arima.df$PHR <- paste0(PHR.arima.df$PHR, ' - ', PHR.arima.df$PHR_Name)
PHR.arima.df$PHR_Name <- NULL
```
...
TSA_Q.arima.df<-TSA.arima.df.1%>%subset(TSA.arima.df.1$TSA=="Q - Greater Houston")
...
write.csv(texas.arima.df.1, 'statistical-output/time-series/state_timeseries.csv', row.names = FALSE)
write.csv(TSA.arima.df.1, 'statistical-output/time-series/tsa_timeseries.csv', row.names = FALSE)
write.csv(county.arima.df.1, 'statistical-output/time-series/county_timeseries.csv', row.names = FALSE)
write.csv(metro.arima.df.1, 'statistical-output/time-series/metro_timeseries.csv', row.names = FALSE)
write.csv(PHR.arima.df, 'statistical-output/time-series/phr_timeseries.csv', row.names = FALSE)
...
# colnames(county.arima.df.2)[1] <- 'Level'
# colnames(metro.arima.df.2)[1] <- 'Level'
# colnames(TSA.arima.df.2)[1] <- 'Level'
# texas.arima.df.2$Level <- 'Texas'

combined.arima.df.1 <- rbind(county.arima.df.1, metro.arima.df.1, TSA.arima.df.1, texas.arima.df.1, PHR.arima.df)
write.csv(combined.arima.df.1, 'statistical-output/time-series/stacked_timeseries.csv',
          row.names = FALSE)

write.csv(combined.arima.df.1, 'tableau/stacked_timeseries.csv',
          row.names = FALSE)
# 
...
```{r}
cumcases.PHR <- PHR.Daily %>% dplyr::select(Date, PHR, Cases_Cumulative)
...
#current_ratio[week1<10] <- NA
current_ratio[current_ratio<0] <- NA
phr_current_ratio_dat <- data.frame(PHR=subset(cumcases.PHR, Date==latestdate, select=PHR)[,1],
                                    current_ratio=current_ratio, current_ratio_cat = cut(current_ratio, breaks=c(0,0.5,.9,1.1,1.5,2,4,8,max(current_ratio))))

# add PHR name
phr_current_ratio_dat <- merge(phr_current_ratio_dat, unique(PHR.Daily[, c('PHR', 'PHR_Name')]), by = 'PHR')

# combine PHR name
phr_current_ratio_dat$PHR <- paste0(phr_current_ratio_dat$PHR, ' - ', phr_current_ratio_dat$PHR_Name)
phr_current_ratio_dat$PHR_Name <- NULL
```
...
```{r}
# declare vals
cumcases.county <- county.Daily %>% dplyr::select(Date,County, Cases_Cumulative)
...
county_current_ratio_dat <- 
  data.frame(County=subset(cumcases.county, Date==latestdate, select=County)[,1],
