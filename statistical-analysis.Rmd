...
covid.arima.forecast<-function(mydata, prediction.period = 10)
{
  ...
  if(max(mydata$Cases_Daily>=5, na.rm = TRUE))
  {
    ...
    arima.out <- data.frame(Date = seq(mindate, maxdate + prediction.period, by = 'days'),
                          ...
                          CI_Lower = rep(NA, times = length(mydata$Date) + prediction.period),
                          CI_Upper =  rep(NA, times = length(mydata$Date) + prediction.period))
    }
...
county.arima.output <- nlme::gapply(county.Daily,
...
metro.arima.output <- nlme::gapply(metro.Daily,
                                   FUN = covid.arima.forecast,
...
```{r TSA forecasts}
TSA.arima.output <- nlme::gapply(TSA.Daily,
                               FUN = covid.arima.forecast,
...
write.csv(texas.arima.df, 'statistical-output/time-series/state_timeseries.csv', row.names = FALSE)
write.csv(TSA.arima.df, 'statistical-output/time-series/tsa_timeseries.csv', row.names = FALSE)
write.csv(county.arima.df, 'statistical-output/time-series/county_timeseries.csv', row.names = FALSE)
write.csv(metro.arima.df, 'statistical-output/time-series/metro_timeseries.csv', row.names = FALSE)
```
...
write.csv(combined.arima.df, 'statistical-output/time-series/stacked_timeseries.csv',
          row.names = FALSE)
```
