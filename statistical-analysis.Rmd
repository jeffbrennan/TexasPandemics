...
Calculate_Ratio = function(df) { 
  ...
  week2 = df %>% 
    filter(Date < latestdate & Date >= middate) %>% 
    group_by(Level_Name) %>%
    summarize(mean(Cases_Daily), na.rm = TRUE) %>% 
    .[, 2] %>%
    unlist()
  ...
  week1 = df %>% 
    filter(Date < middate & Date >= earliestdate) %>% 
    group_by(Level_Name) %>%
    summarize(mean(Cases_Daily), na.rm = TRUE) %>% 
    .[, 2] %>%
    unlist()
  
  current_ratio = week2 / week1
  current_ratio = replace(current_ratio,
...
current_ratio = Calculate_Ratio(casesdaily.TSA)
...
current_ratio = Calculate_Ratio(casesdaily.PHR)
...
current_ratio = Calculate_Ratio(casesdaily.county)
# output
...
current_ratio = Calculate_Ratio(casesdaily.metro)
...
current_ratio = Calculate_Ratio(casesdaily.state)
