...
```{r}
school_dates = seq(as.Date('2020-09-23'), by = 'week', length.out=52)
school_date = max(school_dates[which(school_dates <= date_out)])

school_url = paste0('https://dshs.texas.gov/chs/data/tea/district-level-school-covid-19-case-data/district-level-data-file_',
                          format(school_date, '%m%d%Y'), '.xls')
temp = tempfile()
...
colnames(DSHS_schools_raw)[1] = 'District'
...
district_metadata = read.csv('original-sources/Current_Districts.csv') %>% 
  ...
  mutate(District.Name = gsub('CONSOLIDATED ISD|CISD|CSD', 'CONS ISD', District.Name)) %>% 
  mutate(District.Name = gsub('GOLD BURG', 'GOLD-BURG', District.Name)) %>% 
  mutate(District.Name = gsub('LEGACY PREPARATORY', 'LEGACY PREPARATORY CHARTER ACADEMY', District.Name))


DSHS_schools = DSHS_schools_raw[first_row:last_row, ] %>% 
  group_by(District) %>%
  mutate(LEA = str_match(District, '(\\d{6})')[2]) %>% 
  mutate(District = gsub('CONSOLIDATED ISD|CISD|CSD', 'CONS ISD', District)) %>% 
  mutate(District = gsub('GOLD BURG', 'GOLD-BURG', District)) %>% 
  mutate(District = gsub('Valley View', 'VALLEY VIEW ISD', District)) %>% 
  mutate(District = gsub('TAFORT', 'TAFT', District)) %>%   # assuming FT in taft was converted to tafort - can't find any info about tafort ISD 
  mutate(District = gsub('  ', ' ', District)) %>%
  mutate(District = gsub('\r\n', ' ', District)) %>% 
  merge(., district_metadata, by.x = 'District', by.y = 'District.Name', all.x = TRUE) %>%
  mutate(DISTRICT_N = as.numeric(ifelse(is.na(LEA), DISTRICT_N, LEA))) %>%
  mutate(District = gsub(' \\(LEA.*', '', District)) %>%
  dplyr::select(-NAME, -NCES_DISTR, -LEA) %>% 
  dplyr::select(District, DISTRICT_N, SDLEA10, County.Name, District.City, District.Zip, everything())

DSHS_duplicated_schools = DSHS_schools %>% filter(is.na(County.Name)) %>% 
  merge(., district_metadata, by = 'DISTRICT_N', all.x = TRUE) %>% 
  dplyr::select(-grep('\\.x', names(.))) %>%
  setNames(gsub('\\.y', '', names(.))) %>%
  dplyr::select(-NAME, -NCES_DISTR, -District.Name) %>%
  dplyr::select(District, DISTRICT_N, SDLEA10, County.Name, District.City, District.Zip, everything())

# recombine
DSHS_schools_combined = rbind(DSHS_schools, DSHS_duplicated_schools) %>% 
  filter(!is.na(County.Name))

# both 0
unmatched_districts = DSHS_schools %>% filter(is.na(SDLEA10) & is.na(DISTRICT_N)) %>% filter(grepl('ISD', District))
unmatched_private_schools = DSHS_schools %>% filter(is.na(SDLEA10) & is.na(DISTRICT_N)) %>% filter(!grepl('ISD', District))
```

```{r}
# modify county and cities 
DSHS_schools_combined_clean = DSHS_schools_combined %>%
  ...
  # merge(., merged_city[, 2:3], by.x = 'District.City', by.y = 'City', all.x = TRUE) %>% 
  mutate(Date = school_date) %>%
  dplyr::select(District, Date, DISTRICT_N, SDLEA10, County.Name,
                District.Zip, District.City, everything()) %>% 
  mutate_at(8:ncol(.), as.numeric) %>% 
  setNames(c('District', 'Date', 'District_Number', 'SDLEA10', 'County', 'ZIP', 'City',
             'Enrollment', 'Cases_Weekly_GRADE_EE_3', 'Cases_Weekly_GRADE_4_6', 'Cases_Weekly_GRADE_7_12',
...
setdiff(unique(full_counties), unique(DSHS_schools_combined_clean$County.Name))

# save for future combinations - avoids cleaning on every run
write.csv(DSHS_schools_combined_clean,
...
# legacy_tests = data.frame(readxl::read_excel(temp, sheet = 1))
# legacy_tests_long = legacy_tests %>%
#   setNames(legacy_tests[1, ]) %>%
#   slice(2:(nrow(.) - 11)) %>%
...
#   mutate(Tests_Cumulative = as.integer(as.character(Tests_Cumulative)))
# 
# # store backup
# write.csv(legacy_tests_long, 'original-sources/DSHS_county_tests_legacy_cleaned.csv', row.names = FALSE)
...
```{r}
DSHS_new_cases_long = rbindlist(lapply(list.files(path = 'original-sources/dshs-new-cases', full.names = TRUE),
                                read.csv, fileEncoding="utf-16", sep='\t', stringsAsFactors=FALSE)) %>% 
  dplyr::select(Date, County, New.Cases) %>% 
  setNames(c('Date', 'County', 'Cases_Daily')) %>% 
  mutate(Date = ifelse(as.integer(as.Date(Date, '%m/%d/%Y')) < 0, as.Date(Date, '%m/%d/%y'), as.Date(Date, '%m/%d/%Y'))) %>% 
  mutate(Date = as.Date(Date, origin = '1970-01-01')) %>% 
  mutate(Cases_Daily = as.integer(gsub(',', '', as.character(Cases_Daily), fixed = TRUE))) %>% 
...
DSHS_active_cases_long = DSHS_active_cases_long %>%
  group_by(County) %>% 
  unique() %>%
...
Clean_Childcare = function(df, data_type) { 
  ...
  colnames(grouped_df) = c('County', 'Childcare_Child_Cases_Daily', 'Childcare_Employee_Cases_Daily',
...
# fix all caps county names
county_childcare_df$County = stringr::str_to_title(county_childcare_df$County)

# manually fix exceptions using DSHS_cases_long as dictionary
mismatched_counties = setdiff(unique(county_childcare_df$County), unique(DSHS_cases_long$County))
county_childcare_df$County = gsub('Deafsmith', 'Deaf Smith', county_childcare_df$County)
county_childcare_df$County = gsub('Dewitt', 'DeWitt', county_childcare_df$County)
county_childcare_df$County = gsub('Mcculloch', 'McCulloch', county_childcare_df$County)
county_childcare_df$County = gsub('Mclennan', 'McLennan', county_childcare_df$County)
```
...
# county_counts$Population.x = NULL
# county_counts$Population.y = NULL
```
...
hosp_1 = DSHS_hosp_clean(DSHS_tsa_hosp[[2]], 'Hospitalizations_Total')
hosp_2 = DSHS_hosp_clean(DSHS_tsa_hosp[[3]], 'Hospitalizations_General')
hosp_3 = DSHS_hosp_clean(DSHS_tsa_hosp[[4]], 'Hospitalizations_ICU')
hosp_cap1 = DSHS_hosp_clean(DSHS_tsa_hosp[[5]], 'Beds_Available_Total')
hosp_cap2 = DSHS_hosp_clean(DSHS_tsa_hosp[[6]], 'Beds_Available_ICU')
hosp_cap3 = DSHS_hosp_clean(DSHS_tsa_hosp[[7]], 'Beds_Occupied_Total')
hosp_cap4 = DSHS_hosp_clean(DSHS_tsa_hosp[[8]], 'Beds_Occupied_ICU')
```
...
Clean_HHSC = function(df, data_type) { 
  ...
  na_cols = which(is.na(df[first_row, ]))  # earlier sheets have hidden columns with null values
  
  df = df[first_row:last_row, ]
  
  if(length(na_cols) > 0) {df = df[,  -na_cols]}
  
  colnames(df) = c('HHSC', 'Fac_Outbreak_Active' , 'Fac_Outbreak_Recovered', 'Fac_Outbreak_Cumulative',
                   'Cases_Active', 'Recovered_Cumulative', 'Deaths_Cumulative', 'Cases_Cumulative')
  
  # add data prefix (nursing of ALF (assisted living facility))
...
DSHS_hospitalizations = DSHS_state[[7]][, c(2:3)] %>%
  filter(rowSums(is.na(.)) < ncol(.)) %>% 
...
DSHS_state_day = data.frame(Recovered_Total = DSHS_state[[3]][1,1],
                            ...
                            Antigen_Tests_Total = DSHS_state[[5]][1,2],
                            Antigen_Tests_Positive = DSHS_state[[5]][2,2],
                            Antibody_Tests_Total = DSHS_state[[4]][1,2],
                            Antibody_tests_Positive = DSHS_state[[4]][2,2],
                            Hospital_Bed_Total = DSHS_state[[6]][2,2],
                            Hospital_Bed_Available = DSHS_state[[6]][3,2],
                            ICU_Bed_Available = DSHS_state[[6]][4,2],
                            Ventilator_Available = DSHS_state[[6]][5,2],
                            Case_Investigations = DSHS_state[[8]][[14,2]],
                            Death_Investigations = DSHS_state[[11]][14,2])
```
...
DSHS_demo_clean = function(df) {
  ...
  df = df %>%
    ...
    mutate(var_PCT = as.numeric(gsub('%', '', var_PCT))) %>%
    na.omit()

  # Coerce % to decimal if % found in column values, otherwise just convert char to numeric 
  # 1.1 to account for minor rounding errors
  if(sum(df[['var_PCT']]) > 1.1) {df[['var_PCT']] = df[['var_PCT']] / 100}
...
Demo_Daily = function(df, count_type) { 
  df = df %>% 
    ...
    mutate(var_Daily_NO_NEGATIVE = ifelse(var_Daily < 0, 0, var_Daily))
...
test = DSHS_gender_df %>% filter(Cases_Daily < 0)

DSHS_age_df %>% 
  filter(Date == max(Date)) %>% 
  mutate(manual_pct_cumulative = Cases_Cumulative / sum(Cases_Cumulative)) %>% 
  mutate(manual_pct_daily = Cases_Daily / sum(Cases_Daily)) %>% 
  select(Date, Group, Cases_Cumulative, Cases_Daily, Cases_PCT, manual_pct_cumulative, manual_pct_daily)
...
DSHS_age_df %>% 
  group_by(Date) %>% 
  summarize(sum(Cases_Cumulative), sum(Deaths_Cumulative)) %>% tail(14)

```

## School reopening (DEFUNCT ON 9/24)

```{r}
# TODO: change on 9/23
# school_reopenings = lapply(list.files('original-sources/school-reopening/', full.names = TRUE), read.csv, skipNul = TRUE, sep = '\t', stringsAsFactors = FALSE)
# 
# # staff total and student totals remained the same on 9/23
# school_df = school_reopenings[[1]] %>% 
#   setNames(c('Date', 'Staff_Positive', 'Students_Positive')) %>% 
#   mutate(Date = as.Date(Date, '%B %d, %Y')) %>%
#   unique() %>% 
#   mutate(Staff_Total = ifelse(Date >= as.Date('2020-09-13'), as.numeric(gsub(',', '', school_reopenings[[2]][1,2])), NA)) %>% 
#   mutate(Student_Total = ifelse(Date >= as.Date('2020-09-13'), as.numeric(gsub(',', '', school_reopenings[[3]][1,2])), NA))
# 
# write.csv(school_df, 'tableau/state_school_reopening.csv', row.names = FALSE)
```
