...
county_classifications = read_xlsx('original-sources/metro_classifications.xlsx', sheet = 1)[, c(1, 5, 8)]
...
DSHS_phr = merge(DSHS_phr_counts, DSHS_phr_google, by = c('PHR', 'Date'))
DSHS_phr = merge(DSHS_phr, DSHS_phr_pops, by = 'PHR', all = TRUE)
```
...
```{r}
# phr_out = merge(PHR_computed, DSHS_fac_totals, DSHS_nursing, DSHS_alf, by = c('Date', 'PHR'))
```
...
# DSHS_case_gender <- lapply(demo.list, read_excel, sheet = 13)
# DSHS_case_race <- lapply(demo.list, read_excel, sheet = 13)


# clean all
# for (i in 1:length(DSHS_case_age)) {
#   DSHS_case_age[[i]] = DSHS_case_age[[i]][1:(nrow(DSHS_case_age[[i]])-4), ]
#   
# }
...
```{r}
write.xlsx(DSHS_age, file="combined-datasets/demographics.xlsx",
           sheetName = "age", row.names=FALSE)

write.xlsx(DSHS_gender, file="combined-datasets/demographics.xlsx",
           sheetName = "gender", append=TRUE, row.names=FALSE)

write.xlsx(DSHS_race, file="combined-datasets/demographics.xlsx",
