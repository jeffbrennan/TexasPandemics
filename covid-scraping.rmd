...
DSHS_new_cases_long = rbindlist(lapply(list.files(path = 'original-sources/dshs-new-cases', full.names = TRUE),
                                read.csv, fileEncoding="utf-16", sep='\t')) %>% 
  dplyr::select(Date, County, New.Cases) %>% 
  setNames(c('Date', 'County', 'Cases_Daily_New')) %>% 
  mutate(Date = ifelse(as.integer(as.Date(Date, '%m/%d/%Y')) < 0, as.Date(Date, '%m/%d/%y'), as.Date(Date, '%m/%d/%Y'))) %>% 
  mutate(Date = as.Date(Date, origin = '1970-01-01')) %>% 
  mutate(Cases_Daily_New = gsub(',', '', as.character(Cases_Daily_New), fixed = TRUE)) %>% 
  mutate(Cases_Daily_New = as.integer(Cases_Daily_New)) %>% 
  na.omit()
```
## active cases
...
DSHS_tsa_counts =
    ...
    summarize_at(vars(Cases_Cumulative, Cases_Daily, Cases_Daily_New,
...
hosp_cap2 = DSHS_hosp_clean(DSHS_tsa_hosp_cap[[2]], 'Beds_Occupied_Total')
hosp_cap3 = DSHS_hosp_clean(DSHS_tsa_hosp_cap[[3]], 'Beds_Occupied_ICU')
...
merged_tsa = Reduce(function(x, y) merge(x, y, by = c('Date', 'TSA'), all = TRUE),
                    list(DSHS_tsa, tsa_combined_hosp,
                         ...
                         hosp_cap1, hosp_cap2, hosp_cap3))

# fix types & ensure TSA values are valid
merged_tsa = merged_tsa %>% 
  mutate_at(vars(Beds_Available_Total, Ventilators_Available,
...
state_hosp_detail = 
  ...
  summarize_at(vars(Hospitalizations_Total, Hospitalizations_General, Hospitalizations_ICU,
                    Beds_Available_Total, Beds_Occupied_Total, Beds_Occupied_ICU,
