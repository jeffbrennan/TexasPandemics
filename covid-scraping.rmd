...
```{r}
# DSHS pop
dshs_pops = unique(DSHS_cases_long[, c(1, 3)])
...
HHS_fac_clean = function(df, date) { 
  clean_df = df %>% 
...
HHS_fac_reporting <- data.table::rbindlist(lapply(lapply(hhs_list, read.xlsx, sheet = 1),
                                                  HHS_fac_clean, date=date_out))
```
