...
merged_counts5 = merge(merged_counts4, as.data.frame(census_pops), by = 'County')
...
library(ggplot2)
ggplot(subset(merged_counts5, Date == '2020-06-08'),
       aes(x = County, y = Population_Census / Population_DSHS, color = NCHS_Classification)) + 
    labs(y = 'Census / DSHS Population Ratio') +
    geom_text(aes(label = County), size = 5) +
    ggpubr::theme_pubr() + 
