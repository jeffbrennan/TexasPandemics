...
colnames(JHU_texas_long)[colnames(JHU_texas_long) == 'Value'] = 'Cases'
```
...
# fix column names
colnames(NYT_texas) = c('Date', 'County', 'State', 'FIPS', 'Cases', 'Deaths', 'Source')
```
...
colnames(US_FACTS_texas_long)[colnames(US_FACTS_texas_long) == 'Value'] = 'Cases'
```
...
```{r}
DSHS_counts_county = DSHS_data[[1]]
colnames(DSHS_counts_county) = c('County', 'Cases', 'Deaths')

# add date
current_date = Sys.Date()
DSHS_counts_county$Date = current_date

# drop total row
DSHS_counts_county = DSHS_counts_county[-(which(DSHS_counts_county$County == 'Total')), ]

# add source
DSHS_counts_county$Source = 'DSHS'
```
...
```{r}
CLAFLIN_capacity_20$Capacity = '20'
CLAFLIN_capacity_40$Capacity = '40'
CLAFLIN_capacity_60$Capacity = '60'
...
DSHS_tests = DSHS_tests[1:(nrow(DSHS_tests)-2), ]

colnames(DSHS_tests) = c('Date', 'Viral_Tests', 'Antibody_Tests', 'Total_Tests', 'Positive_Rate')
...
# drop footnote
DSHS_hospitalizations = DSHS_hospitalizations[1:(nrow(DSHS_hospitalizations)-1), ]
...
#rename cols
colnames(DSHS_state_time) = c('Date', 'Viral_Tests', 'Antibody_Tests', 'Tests_Total', 'Positive_Rate', 'Hospital_Total')
...
merged_counts3 = rbind.fill(merged_counts2, DSHS_counts_county)
...
```{r}
write.csv(merged_counts4, file=paste0('combined-datasets/county_counts_', Sys.Date(), '.csv'),
          row.names = F)
```
...
```{r}
write.csv(merged_state4, paste0('combined-datasets/state_tests_hosp_', Sys.Date(), '.csv'), row.names = F)
```

## Additional csvs

```{r}
write.csv(CLAFLIN_texas, paste0('combined-datasets/claflin_capacity.csv'), row.names = F)
write.csv(DSHS_demographics, paste0('combined-datasets/demographics_', Sys.Date(), '.csv'), row.names = F)
```
