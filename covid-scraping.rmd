...
download.file(hosp_cap_url, mode = 'wb',
              destfile = paste0('original-sources/historical/hosp_cap/hosp_cap_', Sys.Date(), '.xlsx'))
```
...
DSHS_json_hosp_tsa$Date = format(Sys.Date(), '%m/%d/%y')
...
# export todays file
write.csv(DSHS_json_hosp_tsa, paste0('original-sources/historical/hosp/tsa_hosp_', Sys.Date(), '.csv'),
          row.names = FALSE)
...
merged_tsa = Reduce(function(x, y) merge(x, y, by = c('Date', 'TSA'), all = TRUE),
                    list(DSHS_tsa, DSHS_hosp_long, tsa_combined_hosp))
```
...
state_hosp_detail = 
  merged_tsa %>% 
  group_by(Date) %>%
  summarize_at(vars(Hospital_Beds_Staffed, Hospital_Beds_Taken,
                    Current_Cases, Hospital_Beds_Available, ICU_Beds_Available,
...
```{r}
library(xlsx)
write.xlsx(merged_state, file="combined-datasets/state.xlsx", sheetName="longitudinal", row.names=FALSE)
...
```{r}
write.xlsx(DSHS_age_df, file="combined-datasets/demographics.xlsx",
           sheetName = "age", row.names=FALSE)

write.xlsx(DSHS_gender_df, file="combined-datasets/demographics.xlsx",
           sheetName = "gender", append=TRUE, row.names=FALSE)

write.xlsx(DSHS_race_df, file="combined-datasets/demographics.xlsx",
           sheetName = "race", append=TRUE, row.names=FALSE)
```
