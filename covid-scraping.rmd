...
tsa_names = readxl::read_xlsx('original-sources/DSHS_tsa_hosp.xlsx')[3:24, 1:2]
colnames(tsa_names) = c('TSA', 'TSA_Name')

tsa_names$TSA = gsub('.', '', tsa_names$TSA, fixed = TRUE)
```
...
merged_dshs = Reduce(function(x, y) merge(x, y, by = 'County', all = TRUE),
                       list(DSHS_county_counts, tsa_names, dshs_pops, county_classifications))
