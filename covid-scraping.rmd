...
```{r}
Clean_County_Tests = function(df) { 
  test_date = as.Date(df[1,2], origin = '1899-12-30')
  colnames(df) = c('County', 'Tests_Cumulative')

  start_row = which(df$County == 'Anderson')
  end_row = which(df$County == 'Zavala')

  clean_df = df[start_row:end_row, ] %>% 
    mutate(Date = test_date)
  
  # archive
  return(list(clean_df, test_date))
}
...
# basic file cleaning
test_clean_output = Clean_County_Tests(data.frame(readxl::read_excel(temp, sheet = 1)))
new_tests = test_clean_output[[1]]

# read in legacy data
legacy_tests = read.csv('original-sources/historical/testing/DSHS_county_tests.csv') %>% 
  dplyr::select('County', 'Tests_Cumulative', 'Date') %>% 
  mutate(Date = as.Date(Date))

# TODO add NA date error catch

# append new tests to old tests
merged_tests = rbind(legacy_tests, new_tests) %>% unique()

# overwrite previous file
if (sum(is.na(merged_tests$Date)) == 0) { 
  write.csv(merged_tests, row.names = FALSE,
          file = 'original-sources/historical/testing/DSHS_county_tests.csv')
}

# calc daily values and fill in df (generated daily on merged file b/c can't calc daily tests with only new data)
DSHS_tests_long = 
  merged_tests %>% 
  group_by(County) %>% 
  tidyr::complete(Date = seq.Date(min(Date), date_out, by="day")) %>%
...
county_counts = Reduce(function(x, y) merge(x, y, by = c('Date', 'County'), all=TRUE),
       list(DSHS_cases_long, DSHS_deaths_long, DSHS_tests_long, DSHS_active_cases_long, county_childcare_df))
# 
...
```{r}
# TODO: refactor (clean once and append to the cleaned files)
# read in all files
