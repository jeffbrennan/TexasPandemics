...
Get_TPR = function() { 
  ...
  tpr_df = read_excel(unzip(temp, list = TRUE)[['Name']][1])
  unlink(temp)
  rm(temp)
  
  
  tpr_df = read_excel(file)
...
if (date_out %in% (TPR_dates + 2)) {Get_TPR()}
...
tpr_cases = merged_county %>% 
  ...
  group_by(County) %>%
  mutate(Cases_100K_7Day = (rollsum(Cases_Daily, k = 7, align = 'right',
                                    na.pad = TRUE, na.rm = TRUE)
                            / Population_DSHS) * 100000) %>% 
  mutate(Cases_100K_7Day_MA = (rollmean(Cases_Daily, k = 7, align = 'right',
                                        na.pad = TRUE, na.rm = TRUE)
                               / Population_DSHS) * 100000) %>%
  mutate(Cases_100K_14Day = (rollsum(Cases_Daily, k = 14, align = 'right',
