...
colnames(DSHS_deaths_time)[2:length(colnames(DSHS_deaths_time))] = dates
...
# fix county names (HARRIS -> Harris) & fuzzy match to case county names
...
hhs_fac_reporting_raw$Date = date_out

# combine csv for archival
openxlsx::write.xlsx(list('1' = hhs_fac_reporting_raw),
...
HHS_fac_clean = function(df) { 
  ...
  # convert numeric date to y-m-d
  clean_df$Date = format(as.Date(clean_df$Date, origin = '1899-12-30'), '%Y-%m-%d')
  return(clean_df)
  }


HHS_fac_reporting <- rbindlist(lapply(lapply(hhs_list, read.xlsx, sheet = 1), HHS_fac_clean))
```
