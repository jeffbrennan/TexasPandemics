...
case_features = subset(mydata, Date >= maxdate-14) %>%
  group_by(County) %>%
  mutate(criteria1 = mean(Cases_Daily_New)) %>% 
  mutate(criteria2 = mean(Cases_Cumulative)) %>% 
  mutate(criteria3 = (mean(Cases_Cumulative) / Population_DSHS) * 1000)%>%
  mutate(criteria4 = max(Cases_Daily_New, na.rm = TRUE)) %>%
  mutate(criteria5 = median(Cases_Daily_New)) %>%
  dplyr::select(County, Date, PHR, TSA, contains('Criteria'))

Plot_Threshold(case_features, 'criteria1', 'mean(Cases_Daily_New)')
...
Plot_Threshold(case_features, 'criteria4', 'max(Cases_Daily_New)')
Plot_Threshold(case_features, 'criteria5', 'median(Cases_Daily_New)')
```
...
## compute stats
```{r}
county_check = lapply(c('Cases_Daily_New', 'Deaths_Daily', 'Tests_Daily', 'Residential'), 
                      calc_count_stats, df = county)

tsa_check = lapply(c('Hospitalizations_Total', 'Hospitalizations_ICU', 'Beds_Available_Total'),
...
tsa_sublevels = c('Hosp_Total', 'Hosp_ICU', 'Cap_Total')
...
level_text = c(rep('County', each=4), rep('TSA', each=3), rep('State', each=3))
...
TMC_df = merged_county_out %>%
  ...
  dplyr::select(County, Population_DSHS, TSA, Cases_Daily_New, Date) %>% 
  arrange(desc(Population_DSHS)) %>% 
