...
calc_count_stats = function(daily_stat, df) { 
  ...
  # TODO: add emoji criteria
  # if declared good, prepend :heavy_check_mark: to text, otherwise :warning:
  ...
  nonzeros = df %>%
    filter(as.Date(Date) == max(as.Date(df$Date))) %>% 
    summarize(!!as.name(daily_stat) > 0 & !is.na(!!as.name(daily_stat))) %>% unlist()

  stats = list(format(max(as.Date(df[['Date']]), '%Y-%m-%d')),
...
county_stats = lapply(c('Cases_Daily', 'Deaths_Daily', 'Tests_Daily', 'Residential'), 
              calc_count_stats, df = county)

tsa_stats = lapply(c('Hospitalizations_Total', 'Hospitalizations_ICU', 'Beds_Available_Total', 'Beds_Available_ICU'),
                   calc_count_stats, df = tsa)

state_stats = lapply(c('Age', 'Gender', 'Race'), calc_demo_stats, df = state)

stats_out = unlist(c(county_stats, tsa_stats, state_stats))
```
...
validation_df = data.frame(Level = c(rep(c('County', 'TSA'), each = 16), rep('State', 9)),
                       ...
                       # Emoji = county_stats$emoji
                           Text = c(rep(c('Latest date:', 'Average daily value:' ,
                                          'Average % change:', '% reporting > 0:'),
                                        times = length(unlist(c(county_stats, tsa_stats))) / 4),
...
build_text = function(text_filters, data_url) { 
  ...
  text_out = validation_df %>% 
    ...
    unite(combined, Text, Statistic, sep = ' ') %>% 
