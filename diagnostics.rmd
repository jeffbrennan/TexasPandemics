...
```{r}
schools = read.csv('tableau/district_school_reopening.csv')
...
```{r}
# cmr tpr 
tpr = readxl::read_excel('original-sources/historical/TPR/10-7.xlsx') %>% 
  setNames(.[6, ]) %>% 
  filter(State == 'TX') %>% 
  mutate(County = gsub(' County, TX', '', County)) %>% 
  dplyr::select(1, 7, 8, 9) %>% 
  mutate_at(c(2,3,4), as.numeric) %>% 
  setNames(c('County', 'Tests', 'Test_Rate', 'TPR_CMS'))
...
county_tpr = county %>% 
  ...
  dplyr::select(County, TSA_Name, Tests_Daily, Cases_Cumulative_daily, Cases_Daily) %>% 
  ...
  reshape2::melt(c('County', 'TSA_Name')) %>% 
  na.omit()
...
ggsave('statistical-output/diagnostics/TPR.png', dpi=800, width = 20, height = 5)
